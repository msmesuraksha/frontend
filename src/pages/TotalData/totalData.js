...
export const TotalData = props => {
    ...
    const selectLoading = useSelector(selectTotalLoading)

    const dispatch = useDispatch()
    useEffect(() => {
        dispatch(fetchTotalTransStart({
    ...
    const viewModel = (value) => {
        ...
        // });
        navigate('/LatestTranaction-View-Details', { state: { selected: value.cell.row.original.totalTrans } });
        ...
        () => [
                ...
                Cell: cellProps => {
                    return (
                        <div className='text-end'>
                            ...
                            {cellProps.cell.row.original != undefined && cellProps.cell.row.original.totalTrans.defaulterEntry != undefined ? numberFormat(cellProps.cell.row.original.totalTrans.defaulterEntry.totalAmount) : ''}
                        </div>
                    );
                },
            },
            ...
            },
            {
                ...
                Cell: cellProps => {
                    return (
                        <div className="d-flex">
                            {cellProps.cell.row.original != undefined ? cellProps.cell.row.original?.totalTrans.defaulterEntry?.status : ""}
                        </div>
                    );
                },
            },
            ...
            },
            {
                ...
                Cell: cellProps => {
                    return (
                        <AssignedComponent value={cellProps.cell.row.original.totalTrans.pHArray} />
                    );
                },
            },
    ...
    const latestTransactiondata = useSelector(selectTotalDataMap)
    ...
    const DueSinceApprove = (cell) => {
        ...
        const valueFordate = cell.row.original.totalTrans.defaulterEntry != null && cell.row.original.totalTrans.defaulterEntry != undefined ? cell.row.original.totalTrans.defaulterEntry.dueFrom : ''
        ...
        return (
            <div className="" style={{ padding: "2px 2px", fontSize: "12px", width: "90px", margin: "0px" }}>
                <div className=" text-center  p-1 rounded " style={{
                    ...
                    <div className="text-capitalize" >{cell.row.original.totalTrans.defaulterEntry == null ? '' : cell.row.original.totalTrans.defaulterEntry.dueFrom}</div>
                </div>
            </div>
        );
    };
