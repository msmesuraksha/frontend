...
import {
  selectTotalDataMap,
  selectTotalLoading,
} from "store/totalDataTRansaction/totalDataTRansaction.selecter"
...
export const TotalData = props => {
  ...
  const viewModel = value => {
    ...
    navigate("/LatestTranaction-View-Details", {
      state: { selected: value.cell.row.original.totalTrans },
    })
    ...
    () => [
        ...
        Cell: cellProps => {
          return (
            <div className="text-end">
              ...
              cellProps.cell.row.original.totalTrans.defaulterEntry != undefined
                ? numberFormat(
                    cellProps.cell.row.original.totalTrans.defaulterEntry
                      .totalAmount
                  )
                : ""}
            </div>
...

      {
        ...
        Cell: cellProps => {
          ...
          return <div className="d-flex">{currentStatus}</div>
        },
      },
      {
        ...
        Cell: cellProps => {
          return (
            <div className="d-flex">
              {cellProps.cell.row.original != undefined
                ? cellProps.cell.row.original?.totalTrans.defaulterEntry?.status
                : ""}
            </div>
          )
        },
      ...
      },
      {
        ...
        Cell: cellProps => {
          return (
            <AssignedComponent
              value={cellProps.cell.row.original.totalTrans.pHArray}
            />
          )
        },
      },
    ...
    return (
      <div className="d-flex">
        {uniqeArray.map((newvalue, index) => {
          return (
            <span key={index}>{`${index > 0 ? "," : ""} ${newvalue}`}</span>
          )
        })}
      </div>
    )
    ...
    if (latestTransactiondata) {
      if (filters === "") {
        ...
        const filteredResults = latestTransactiondata.filter(item => {
          return item?.defaulterEntry?.creditor?.companyName
            .toLocaleLowerCase()
            .includes(filters)
        })
  ...
  const DueSinceApprove = cell => {
    const valueFordate =
      cell.row.original.totalTrans.defaulterEntry != null &&
      cell.row.original.totalTrans.defaulterEntry != undefined
        ? cell.row.original.totalTrans.defaulterEntry.dueFrom
        : ""
        ...
        <Container fluid={true}>
          ...
          <Card>
            <CardBody>
              ...
              <p className="">
                Due Since : The number of due days is calculated from date of
                oldest invoice.
              </p>
              <p> &nbsp;</p>
            </CardBody>
