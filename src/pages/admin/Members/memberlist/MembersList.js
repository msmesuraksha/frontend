    ...
    () => [
      ...
      },
      {
        ...
        Cell: cellProps => {
          return (
            <div>
              {cellProps.row.original?.isActiveAccount == "ACTIVE" && (
                <b style={{ color: "#5cd65c" }}>
                  {cellProps.row.original?.isActiveAccount}
                </b>
              )}
              {cellProps.row.original?.isActiveAccount == "INACTIVE" && (
                <b style={{ color: "#ff1a1a" }}>
                  {cellProps.row.original?.isActiveAccount}
                </b>
              )}
            </div>
          ...
          return (
            <div className="d-flex">
              <div
                ...
                onClick={() => handleViewDetails(cellProps.cell.row)}
                style={{ cursor: "pointer" }}
              >
                <i
                  className="mdi mdi-eye font-size-16 text-primary me-1"
              ...
              <div
                className="d-flex flex-column align-items-center me-3"
                onClick={() => handleActiveBtn(cellProps.cell.row.original.id)}
                style={{ cursor: "pointer" }}
              >
                <i
                  className="mdi mdi-play font-size-18 text-success mb-1"
                  data-bs-toggle="tooltip"
                  data-bs-placement="top"
                  title="Activate"
                />
              ...
              <div
                className="d-flex flex-column align-items-center me-3"
                onClick={() =>
                  handleDeactivateBtn(cellProps.cell.row.original.id)
                }
                style={{ cursor: "pointer" }}
              >
                <i
                  className="mdi mdi-pause font-size-18 text-success mb-1"
                  data-bs-toggle="tooltip"
                  data-bs-placement="top"
                  title="Deactivate"
                />
            ...
            <CardBody>
              <TableContainer
                columns={
                  adminRole != "L1"
                    ? columns
                    : columns.filter(value => value.Header != "Action")
                }
                data={
                  memberData != undefined && memberData != [] ? memberData : []
                }
                isGlobalFilter={true}
                isAddOptions={false}
                customPageSize={20}
                ExportFile={true}
                url={"/api/admin/downloadAllUsers"}
                fileName={"AllMemberList"}
              />
            </CardBody>
          </Card>
        </Container>
